...
const Calendario = ({
  ...
  <li className="flex flex-col items-center justify-between p-4 border-b-[1px] border-[#022933] h-auto calendario">
    ...
    >
        ...
        <div className="flex justify-around mb-0">
          ...
          <button
            ...
            className={`w-1/2 py-2 rounded-t-3xl ${
              showSalida ? "bg-[#88A198] text-white" : "bg-[#F4EFDF] text-black"
            }`}
          >
            Salida
          </button>
      ...
      {/* Para versión móvil */}
      <div
        className={`md:hidden relative mobile-datepicker transition-all duration-500 ease-in-out ${
          showSalida ? "h-[500px]" : "h-[500px]"
        }`}
      ...
      >
        ...
        <div
          className={`absolute left-0 right-0 transition-all w-[300px] duration-500 ease-in-out ${
            showSalida ? "-translate-y-8 z-0" : "translate-y-20 z-20"
          }`}
        >
