...
const Resumen = ({ onToggleResumen }) => {
  ...
  const handleToggle = () => {
    ...
    onToggleResumen(!showDetails);  // Notifica al componente padre si se abrió o cerró
  };
    ...
    <li className="flex flex-col items-center p-4 border-b-[1px] border-[#022933] resumen">
      <div className="flex items-center justify-between w-full h-[5vh] sm:h-full">
        ...
        <button onClick={handleToggle}>
          <img
            className={`w-4 transition-transform duration-500 ${showDetails ? "rotate-180" : "rotate-0"}`}
      ...
      <div className={`w-full mt-4 flex flex-col items-center justify-center overflow-hidden transition-all duration-500 ease-in-out ${showDetails ? "max-h-[1000px] opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="w-[95%] sm:w-[65%] border border-black rounded-3xl">
          <div className="bg-[#383838] rounded-t-2xl py-6 text-white text-center">Confirma y paga</div>
          <div className="w-full flex justify-center">
            <div className="p-4 w-[80%] flex items-center justify-between border-b border-black border-opacity-25">
              ...
              <span className="creatoLight">Entras 14 agosto<br />Sales 29 agosto</span>
            </div>
          </div>
          <div className="w-full flex justify-center">
